...
group <- ifelse(grepl("control", metadata$`disease state:ch1`, ignore.case = TRUE), "control", "endometriosis")
group <- factor(group)
table(group)
...
results$GeneSymbol <- getSYMBOL(rownames(results), "illuminaHumanv4.db")
head(results)
...
write_xlsx(up_genes, "deg_GSE23339_up_genes.xlsx")
write_xlsx(down_genes, "deg_GSE23339_down_genes.xlsx")
...
write_xlsx(as.data.frame(go_result_BP), "deg_GSE23339_GO_BP_enrichment.xlsx")
...
write_xlsx(as.data.frame(go_result_MF), "deg_GSE23339_GO_MF_enrichment.xlsx")
...
write_xlsx(as.data.frame(go_result_CC), "deg_GSE23339_GO_CC_enrichment.xlsx")
...
write_xlsx(as.data.frame(reactome_enrich), "deg_GSE23339_reactome_enrichment.xlsx")
...
pheatmap(xcell_result, 
         main = "xCell Immune Infiltration - GSE23339",
         scale = "row",
         clustering_distance_rows = "euclidean",
         clustering_distance_cols = "euclidean")

write_xlsx(xcell_df, "deg_GSE23339_xCell_Results.xlsx")
...
write_xlsx(stat_tests, "deg_GSE23339_xcell_stat_tests.xlsx")
...
ggplot(long_xcell, aes(x = group, y = proportion, fill = group)) +
  ...
  theme_minimal() +
  labs(title = "Immune Cell Infiltration (xCell) - GSE23339") +
