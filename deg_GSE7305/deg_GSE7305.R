...
group <- ifelse(grepl("normal", metadata$characteristics_ch1, ignore.case = TRUE), "normal", "endometriosis")
group <- factor(group)
table(group)
...
head(deg)
write_xlsx(deg, "deg_GSE7305.xlsx")
...
write_xlsx(up_genes, "deg_GSE7305_up_genes.xlsx")
write_xlsx(down_genes, "deg_GSE7305_down_genes.xlsx")
...
write_xlsx(as.data.frame(go_result_BP), "deg_GSE7305_GO_BP_enrichment.xlsx")
#GO_MF
...
write_xlsx(as.data.frame(go_result_MF), "deg_GSE7305_GO_MF_enrichment.xlsx")
#GO_CC
...
write_xlsx(as.data.frame(go_result_CC), "deg_GSE7305_GO_CC_enrichment.xlsx")
#KEGG enrichment
...
write_xlsx(as.data.frame(kegg_enrich), "deg_GSE73058_KEGG_enrichment.xlsx")
...
write_xlsx(as.data.frame(reactome_enrich), "deg_GSE7305_reactome_enrichment.xlsx")
...
pheatmap(xcell_result, 
         main = "xCell Immune Infiltration - GSE7305",
         scale = "row",
         clustering_distance_rows = "euclidean",
         clustering_distance_cols = "euclidean")
write_xlsx(xcell_df, "deg_GSE7305_xCell_Results.xlsx")
...
stat_tests <- long_xcell %>%
  ...
  arrange(p_value)
write_xlsx(stat_tests, "deg_GSE7305_xcell_stat_tests.xlsx")

ggplot(long_xcell, aes(x = group, y = proportion, fill = group)) +
  geom_boxplot(outlier.shape = NA) +
  facet_wrap(~ cell_type, scales = "free_y") +
  theme_minimal() +
  labs(title = "Immune Cell Infiltration (xCell) - GSE7305") +
